  ...
  return (
    <TiltCard className="group rounded-2xl sm:rounded-3xl overflow-hidden flex flex-col relative cursor-pointer shadow-2xl border border-white/10 bg-[#0a0a0f]/95 backdrop-blur-2xl">
      ...
      <div className="flex flex-col md:flex-row relative z-10 min-h-0 md:min-h-[500px]">
        <div
          className={cn(
            "p-6 sm:p-8 md:p-12 flex-1 flex flex-col justify-between order-2 md:order-1",
            hasImage && "md:border-r border-white/10"
          )}
        >
          <div>
            <div className="flex justify-between items-start gap-3 mb-4 sm:mb-6">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold group-hover:text-primary transition-colors">
                {project.title}
              </h3>
          ...
          <div className="flex flex-wrap gap-2 mt-8 sm:mt-12">
        ...
        {hasImage && (
          <div className="w-full md:w-[50%] lg:w-[55%] p-6 sm:p-8 md:p-12 flex flex-col items-center justify-between bg-[#030305] order-1 md:order-2 relative overflow-hidden">
            ...
            <div className="w-full max-w-[16rem] sm:max-w-[18rem] md:max-w-md lg:max-w-lg relative z-10 group-hover:-translate-y-2 transition-transform duration-500 mb-6 sm:mb-10">
...
function StackedProjectCard({
  ...
  const top = `calc(10vh + ${idx * 20}px)`;

  return (
    <div
      className={cn("sticky w-full flex justify-center", isLast ? "mb-0" : "mb-[32vh] lg:mb-[40vh]")}
      style={{ top }}
    >
      <motion.div
...
export const Projects = () => {
  ...
  return (
    <section id="projects" ref={containerRef} className="py-12 md:py-24 relative overflow-x-clip">
      <div className="container mx-auto px-4 sm:px-6 md:px-12">
        ...
        {/* Mobile / tablet: simple vertical list (sticky stack breaks on small screens) */}
        <div className="relative z-10 mt-6 sm:mt-10 flex flex-col gap-6 sm:gap-8 md:hidden">
          {PROJECTS.map((project, idx) => (
            ...
            >
              <div className="w-full max-w-6xl drop-shadow-2xl">
                <ProjectCardContent project={project} />
              </div>
            </motion.div>
          ))}
        ...
        {/* md+: scroll stack */}
        <div className="relative z-10 mt-6 sm:mt-10 hidden md:block pb-0">
          {PROJECTS.map((project, idx) => (
            <StackedProjectCard
              key={project.title}
              project={project}
